...
| --- | --- | --- |
| `OfficeLocation` | Saved office coordinates + capture time. | The persisted unit (AND-07). |
| `DeviceLocation` | Current position + accuracy + timestamp. | Carries accuracy so quality can be judged (AMB-14). |
...
| `ProximityResult` | Distance in metres + in-range flag. | Feeds both the gauge (AND-17) and the button gate (AND-08). |

The 50 m radius is a single named constant in `domain`. Nothing else may hard-code it.

#### `data` — device and persistence access

| Element | Responsibility | Notes |
| --- | --- | --- |
| `LocationDataSource` | Wraps `FusedLocationProviderClient`. Streaming updates as a `callbackFlow`; a separate one-shot current-location call for "Set Office Location" (AND-06). | High-accuracy priority ([ADR-001](DECISIONS.md#adr-001)). Cancellation must remove the callback. |
...
`AttendanceViewModel` combines three inputs — permission status, office location
`Flow`, and device location `Flow` — into **one** `StateFlow<AttendanceUiState>`
([ADR-006](DECISIONS.md#adr-006)).

`AttendanceUiState` models the screen's conditions as a **sealed hierarchy**, not as
...
└── Tracking(distance, inRange, accuracy, quality)
```
