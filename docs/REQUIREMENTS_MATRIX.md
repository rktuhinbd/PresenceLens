...
Evidence recorded at G3 (2026-08-28): `testDebugUnitTest` 56/56 PASS, `assembleDebug`
PASS, `lintDebug` 0 errors. Rows whose verification method is a **Compose UI test** or a
**device/emulator check** stay `PARTIAL` until that specific method is executed — a
passing JVM suite is not evidence for them.
...
| AND-03 | Create `AttendanceScreen`. | p1, Task 1 intro | Composable named exactly `AttendanceScreen` in `presentation/attendance`, stateless; `AttendanceRoute` holds the ViewModel. | Symbol exists with that name; Compose UI test targets it. **Symbol exists and is rendered by 12 `@Preview` states**; the Compose UI test is not written (device gate). | PARTIAL |
...
| AND-05 | Setup Phase: a button labelled "Set Office Location". | p1, Setup Phase | `OutlinedButton` bound to `R.string.set_office_location`, whose value is exactly `Set Office Location`. The label is deliberately **not** swapped while capturing or after the office is set. | Compose UI test asserts node with text "Set Office Location". **String resource verified exact**; the UI test is not written (device gate). | PARTIAL |
| AND-06 | That button must **fetch the current GPS coordinates**. | p1, Setup Phase | `LocationDataSource.currentLocation()` — a one-shot `getCurrentLocation` at high accuracy with a 2 s max cached-fix age, deliberately distinct from the streaming updates so a stale position is never recorded. | Instrumented/manual: pressing it captures and displays live coordinates. **`AttendanceViewModelTest` asserts a fresh one-shot request is made and that its coordinate is what gets persisted**; the device check is outstanding. | PARTIAL |
| AND-07 | Coordinates must be **saved locally**. | p1, Setup Phase | DataStore Preferences (`office_lat`, `office_lon`, `captured_at`) via `OfficeLocationRepository` / `DataStoreOfficeLocationRepository`, now wired into the live ViewModel. See [ADR-002](DECISIONS.md#adr-002). | Set location → force-stop → relaunch → coordinates still present. **Persistence layer verified by JVM unit tests** (file-backed round trip; in-memory overwrite/clear; unreadable-store recovery). The force-stop/relaunch device check is outstanding. | PARTIAL |
| AND-08 | "Mark Attendance" enabled/functional **only** when current location is within a **50-metre radius** of the saved coordinates. | p1, Validation Phase | Pure `AttendanceRule` in `domain`; `AttendanceUiState.canMarkAttendance` is its only consumer, and `onMarkAttendanceClicked` re-evaluates the rule on the click path so a stale frame cannot let a tap through. | Unit tests at 0 m, 49.9 m, exactly 50 m, 50.1 m, 120 m + UI test that the button is disabled out of range. **Rule boundary tests pass, and `AttendanceViewModelTest` asserts enablement at 49.9 / 50.0 / 50.1 m plus the out-of-range click being a no-op.** The UI-test half is not written (device gate). | PARTIAL |
| AND-09 | Feedback: **real-time** distance indicator (e.g. "You are 120m away from the office"). | p1, Feedback | Distance recomputed on every location emission; rendered as "You are 47 m away from the office" alongside the gauge readout. `DistanceFormatter` handles metres and kilometres. | Manual emulator walk-in: value updates without user action; UI test on formatted string. **`DistanceCalculator`, `DistanceFormatter`, and per-emission recomputation are all unit-tested**; the live emulator walk is outstanding. | PARTIAL |
| AND-10 | UI must follow the provided screenshot. | p1, UI bullet + p2 screenshot | Compose implementation preserving the p2 reference information architecture, ordering, controls, and overall composition (AND-13…AND-21), executed to the quality bar in [ADR-012](DECISIONS.md#adr-012). | Side-by-side comparison against the p2 reference, recorded in README §5; ADR-012 constraints checked at G3. **ADR-012 constraints checked** — stable Material 3 only, no alpha or preview design libraries on the classpath, no requirement distorted. The side-by-side capture is outstanding. | PARTIAL |
...
| AND-12 | Android state management via **Kotlin Flow**. | p1, General Requirements b1 (applied to Android) | `AttendanceViewModel` exposes a single `StateFlow<AttendanceUiState>` built by `combine` over four sources; location is a cold `callbackFlow`. | Unit test drives the flow and asserts emitted state sequence. **Executed — 19 `AttendanceViewModelTest` cases drive the flow through permission, service, fix, office, and action transitions.** | DONE |
| AND-13 | `[screenshot-derived]` Top app bar: back affordance + title "Attendance". | p2 screenshot | Material 3 `TopAppBar` with a project-owned back icon and the title. | Visual comparison. Implemented; capture outstanding. | PARTIAL |
| AND-14 | `[screenshot-derived]` "STEP 1: OFFICE CONTEXT" card with a status dot. | p2 screenshot | `OfficeContextCard` with the overline and an animated state-coloured `StatusDot` carrying its own content description. | Visual comparison. Implemented; capture outstanding. | PARTIAL |
| AND-15 | `[screenshot-derived]` Map/location visual with a coordinate readout pill ("Lat: …, Lon: …"). | p2 screenshot | **No Google Maps SDK** ([ADR-003](DECISIONS.md#adr-003)). `LocationSurface` is an original Compose-drawn plan view: guide rings, the 50 m boundary, the office at centre, and the live position at its true bearing on a compressed radial scale, with the coordinate pill overlaid. No branding, no tiles, no key, no pan/zoom affordance. | Visual comparison; coordinate pill shows persisted values; renders fully on a clean clone with no key configured. **No Maps dependency exists in the module and the surface needs no network or key — verified by dependency inspection and a clean `assembleDebug`.** `ProximityGeometry` placement maths is unit-tested. Visual capture outstanding. | PARTIAL |
| AND-16 | `[screenshot-derived]` Helper copy above the button: office location must be correctly identified. | p2 screenshot | Supporting text in the setup card, above the button. | Visual comparison. Implemented; capture outstanding. | PARTIAL |
| AND-17 | `[screenshot-derived]` Circular distance gauge: large "120m" + "AWAY" caption + coloured progress arc. | p2 screenshot | `DistanceGauge`: Compose `Canvas` ring whose arc is driven by `ProximityGeometry.radiusUsageFraction`, coloured by the same eligibility decision the button uses. | Visual comparison; gauge fraction unit-tested. **Gauge fraction unit-tested (`ProximityGeometryTest`)**; visual capture outstanding. | PARTIAL |
| AND-18 | `[screenshot-derived]` Range status chip ("OUT OF RANGE", red) with an in-range counterpart. | p2 screenshot | `RangeStatusChip` bound to `AttendanceUiState.canMarkAttendance` — the same value the button reads, so the two cannot disagree. Error tones out of range, success tones in. | Visual comparison; UI test for both states. **Both states rendered in previews**; the UI test is not written (device gate). | PARTIAL |
| AND-19 | `[screenshot-derived]` Guidance copy: "Move within 50 meters of the designated office location to enable check-in." | p2 screenshot | Supporting text under the chip when out of range, with the radius supplied from `AttendanceRule.ELIGIBLE_RADIUS_METERS` rather than hard-coded in the string. | Visual comparison. Implemented; capture outstanding. | PARTIAL |
| AND-20 | `[screenshot-derived]` Locked attendance region: dashed container + lock icon + disabled "Mark Attendance" button. | p2 screenshot | `AttendanceActionPanel`: one container whose dashed outline resolves to solid and whose lock becomes a check when the region unlocks; the button's disabled state is explicit, and the locked reason is announced to screen readers. | Visual comparison; UI test on enabled/disabled. **Enablement logic unit-tested via `canMarkAttendance`; both states rendered in previews.** The UI test is not written (device gate). | PARTIAL |
| AND-21 | `[screenshot-derived]` Caption "AVAILABLE 09:00 AM - 10:30 AM". **Presentation detail only.** | p2 screenshot only; **no sentence in the PDF requires a time window** | Rendered in `AttendanceActionPanel` for fidelity. It **must never become an attendance eligibility rule** ([ADR-011](DECISIONS.md#adr-011)). | Visual comparison; **plus a G3 code review confirming no path consults the availability window when computing button enablement**. **Code review executed 2026-08-28 — PASS:** `R.string.availability_caption` has exactly one reference in the codebase, a `Text` call; `canMarkAttendance` reads only `AttendanceRule`; no clock or time value participates in enablement anywhere. Visual capture outstanding. | PARTIAL |
