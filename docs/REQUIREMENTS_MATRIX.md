...
| AND-07 | Coordinates must be **saved locally**. | p1, Setup Phase | DataStore Preferences (`office_lat`, `office_lon`, `captured_at`) via `OfficeLocationRepository` / `DataStoreOfficeLocationRepository`. See [ADR-002](DECISIONS.md#adr-002). | Set location → force-stop → relaunch → coordinates still present. **Persistence layer verified by JVM unit test** (initially absent, save-and-read-back, overwrite, clear via a temp-file DataStore); the force-stop/relaunch device check is still outstanding once wired into the real app (G3). | PARTIAL |
| AND-08 | "Mark Attendance" enabled/functional **only** when current location is within a **50-metre radius** of the saved coordinates. | p1, Validation Phase | Pure `AttendanceRule` in domain layer returning in-range/out-of-range from (current, office, 50 m); button `enabled` bound to it **and** the click path re-checks. | Unit tests at 0 m, 49.9 m, exactly 50 m, 50.1 m, 120 m + UI test that the button is disabled out of range. **Rule + boundary unit tests done and passing**; the UI-test half is not started (no `AttendanceScreen` yet). | PARTIAL |
| AND-09 | Feedback: **real-time** distance indicator (e.g. "You are 120m away from the office"). | p1, Feedback | Distance recomputed on every location emission and rendered continuously. | Manual emulator walk-in: value updates without user action; UI test on formatted string. **Underlying `DistanceCalculator` (Haversine) unit-tested** (identity, known-distance, symmetry); the live-emission and rendering behaviour is not started. | PARTIAL |
