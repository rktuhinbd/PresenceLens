...
**Status as of gates F4 and F5 (2026-08-30).** The production experience is built:
...
| FLT-GEN-004 | The app shall handle permission and hardware failures gracefully. | p1 GR-4 | MANDATORY | `CameraCubit`, `CameraAdapter` | `BLOC`, `WIDGET`, `DEVICE` | PARTIAL — every permission and hardware failure is now a **rendered** state as well as a classified one: `WIDGET` executed over refusal, repeated refusal, no rear camera, initialisation failure and enumeration failure, each keeping the Pending Uploads entry reachable. `DEVICE` outstanding. |
...
| FLT-CAM-002 | The screen shall render a live in-app camera preview (not a system camera intent). | p2 Custom Camera UI | MANDATORY | `CameraPreviewScreen` | `DEVICE` | PARTIAL — the screen renders `buildCameraPreview` over the live session, full-bleed, with no system camera intent anywhere in the app. A fake session degrades to the placeholder, which is what makes the screen testable — and is **not** evidence that a preview works. `DEVICE` outstanding. |
| FLT-CAM-003 | Zoom shall be adjustable by pinch gesture. | p2 Zoom | MANDATORY | `CameraPreviewScreen`, `ZoomPolicy` | `UNIT` (gesture→zoom mapping), `DEVICE` | PARTIAL — `UNIT` and `BLOC` as at F3, plus `WIDGET`: a two-finger outward drag on the viewport raises `CameraCubit.currentZoom` and the slider re-renders from that same number. `DEVICE` check 6 outstanding. |
| FLT-CAM-004 | Zoom shall be adjustable by an on-screen slider. | p2 Zoom | MANDATORY | `ZoomSlider` | `WIDGET`, `DEVICE` | PARTIAL — `WIDGET` executed: `ZoomSlider` renders only when the camera reports an adjustable range, is bounded by the reported min/max, and writes the one shared `currentZoom`, which then reaches the platform. `DEVICE` outstanding. |
| FLT-CAM-005 | Zoom shall offer rounded preset buttons derived from the device's reported capabilities. | p2 Zoom (**text truncated — root AMB-01**) | MANDATORY | `ZoomPresetPolicy` | `UNIT`, `DEVICE` | PARTIAL — `UNIT` as at F3, plus `WIDGET`: the row is absent on a fixed-zoom camera, and a camera reporting 1–3 offers `1x` and `2x` only - 5x and 10x are outside what it reported and are not shown. `DEVICE` checks 2–4, 8 outstanding. |
...
| FLT-CAM-008 | Tapping the preview shall set the camera focus point. | p3 Manual Focus | MANDATORY | `CameraCubit`, `FocusPointMapper` | `UNIT` (coordinate normalisation), `DEVICE` | PARTIAL — `UNIT` as at F3, plus `WIDGET`: a tap on the viewport produces exactly one normalised focus point through the real mapper against the geometry the widget supplied, and pairs the exposure point because that camera reported support for it. `DEVICE` checks 9–10 outstanding. |
| FLT-CAM-009 | A visual indicator shall appear at the tapped point. | p3 Manual Focus | MANDATORY | `FocusReticle` | `WIDGET` (asserts position), `DEVICE` | PARTIAL — `WIDGET` executed: the reticle's rendered centre is within 2 logical pixels of the tap, because it is positioned in **widget** coordinates rather than round-tripped back through the normalisation. `DEVICE` outstanding. |
...
| FLT-CAM-011 | The app shall enumerate cameras and present only back-facing cameras for selection. | derived from p2 "available back cameras" | MANDATORY | `CameraAdapter` | `UNIT`, `DEVICE` | PARTIAL — `UNIT` as at F3, plus `WIDGET`: one rear camera renders no selector at all, and two render one whose accessible value is an **ordinal** (“Camera 1, 1 of 2”) containing no fabricated multiplier. `DEVICE` check 3 outstanding. |
| FLT-CAM-012 | The app shall own the controller lifecycle: dispose on `paused`/`inactive`, reinitialise on `resumed`. | RESEARCH `FR-02`; GR-4 | MANDATORY | `CameraPreviewScreen` | `WIDGET`, `DEVICE` | PARTIAL — `BLOC` as at F3, and the observer now exists: `CameraPreviewScreen` implements `WidgetsBindingObserver` and translates `AppLifecycleState` into a `CameraLifecycleSignal`, adding no decision of its own. `DEVICE` check 14 outstanding. |
...
| FLT-BAT-003 | The app shall show a list of "Pending Uploads". | p3 Batch Management | MANDATORY | `UploadManagerScreen` | `WIDGET`, `DEVICE` | PARTIAL — `WIDGET` executed: `UploadManagerScreen` renders batch sections with count-based `n of m` progress, per-item rows, the connectivity chip, the reassurance line and the empty state. `DEVICE` outstanding. |
...
| FLT-ERR-002 | Permanent denial shall route the user to the OS app settings. | GR-4 | MANDATORY | `CameraPreviewScreen` | `DEVICE` | PARTIAL — the route is built and exercised: a `MethodChannel` in `MainActivity` fires `ACTION_APPLICATION_DETAILS_SETTINGS`, offered only after refusals repeat and never worded as a permanence claim (`ADR-F22`); a `WIDGET` test proves the offer appears on the second refusal and reaches the launcher. Its stated method is `DEVICE`, which is outstanding. |
...
| ID | Requirement | Source | Priority | Component | Verification | Status |
| --- | --- | --- | --- | --- | --- | --- |
| FLT-UX-001 | The camera preview shall be the dominant surface; chrome shall not obstruct it unnecessarily. | derived; p3 left | QUALITY | `CameraPreviewScreen` | `REVIEW`, `DEVICE` | PARTIAL — `REVIEW` executed against the built screen: gradient scrims rather than panels, contextual controls, nothing in the optical centre, and an asserted absence of any close control. `DEVICE` outstanding. |
